...
- **Attendee Management**: Enhanced module with interest tracking, notes, and category classification (Practitioner, Gov/Non-Profit, Solution Provider). Admin table includes `AttendeeDetailDrawer` with category/ticketType display, category column in the admin table, and an Attendee Category Breakdown pie chart that filters by selected event tab.
