...
- **Attendee Concierge Portal (Phase A)**: Passwordless token-based attendee access portal. Admin generates concierge links via `POST /api/admin/attendees/:id/generate-concierge-link` (requireAuth). Token auto-login at `/attendee-access/:token` stores token to localStorage and redirects to `/attendee`. Portal at `/attendee` shows: (1) Welcome onboarding step with event name and feature summary; (2) Topic chip multi-select (up to 5 topics, submitted topics mark onboarding complete); (3) Dashboard with interests summary (editable), recommended sessions, and recommended sponsors — both recommendation lists scored by topic overlap with the attendee's selections, fallback shows all event items when no topics selected. Schema: `attendee_tokens` (token, attendeeId, eventId, isActive, expiresAt, onboardingCompletedAt, onboardingSkippedAt). Storage: `createAttendeeToken`, `getAttendeeToken`, `getAttendeeTokensByAttendee`, `updateAttendeeToken`. API routes: `GET /api/attendee-access/:token`, `GET /api/attendee-portal/me|topics|topic-selections|recommended-sessions|recommended-sponsors`, `POST /api/attendee-portal/topic-selections|skip-onboarding`. Token passed via `x-attendee-token` header or `?token=` query param.
