...
- **Attendee Concierge Portal (Phase A + B)**: Passwordless token-based attendee access portal. Admin generates concierge links via `POST /api/admin/attendees/:id/generate-concierge-link` (requireAuth). Token auto-login at `/attendee-access/:token` stores token to localStorage and redirects to `/attendee`. **Phase A** — Portal at `/attendee` shows: (1) Welcome onboarding step with event name and feature summary; (2) Topic chip multi-select (up to 5 topics, submitted topics mark onboarding complete); (3) Dashboard with interests summary (editable), recommended sessions (save/unsave), and recommended sponsors — both recommendation lists scored by topic overlap with the attendee's selections, fallback shows all event items when no topics selected. **Phase B** — Full event agenda browsing at `/attendee/agenda` (sessions grouped by day, filtered by day/type, save/unsave toggle, session detail slide-over); My Agenda at `/attendee/my-agenda` (saved sessions grouped by day, remove, per-session and bulk ICS download); Sponsors listing at `/attendee/sponsors`. Navigation via `AttendeeShell` tab bar (Home / Agenda / My Agenda / Sponsors). Schema: `attendee_tokens` (token, attendeeId, eventId, isActive, expiresAt, onboardingCompletedAt, onboardingSkippedAt). Storage: `createAttendeeToken`, `getAttendeeToken`, `getAttendeeTokensByAttendee`, `updateAttendeeToken`, `getAttendeeSavedSessions`, `createAttendeeSavedSession`, `deleteAttendeeSavedSession`, `countAttendeeSavedSessions`. API routes: attendee-portal — `me|topics|topic-selections|recommended-sessions|recommended-sponsors|agenda|saved-sessions|my-agenda/ics|skip-onboarding`; admin — `GET /api/admin/attendees/:id/saved-session-count`, `POST /api/admin/attendees/saved-session-counts-batch`, `GET /api/admin/attendees/:id/saved-sessions`. Token passed via `x-attendee-token` header or `?token=` query param. ICS bulk download uses `?token=` query param. Key components: `useAttendeeAuth.ts`, `AttendeeShell.tsx`, `SessionDetailSheet.tsx`, `AttendeeAgendaPage.tsx`, `AttendeeMyAgendaPage.tsx`, `AttendeeSponsorsPage.tsx`. **Admin visibility**: AttendeesTable shows a calendar icon button per attendee row (teal accent, disabled when count = 0, numeric badge overlay when count > 0); clicking opens `AdminAttendeeAgendaSheet` showing the attendee's saved sessions grouped by date. Counts fetched in batch via `POST /api/admin/attendees/saved-session-counts-batch`.
