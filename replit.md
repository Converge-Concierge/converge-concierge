...
- **Configuration**: Category Rules, Branding, Settings, Users, Access Control, Sponsorship Templates
...
- **Attendee Management**: Enhanced module with interest tracking, notes, and category classification. Admin table includes `AttendeeDetailDrawer` with category/ticketType display, category column in the admin table, and an Attendee Category Breakdown pie chart that filters by selected event tab.
- **Attendee Category Rules Engine**: DB-driven category classification system replacing hardcoded logic. `attendee_categories` table defines category labels with matchmaking weights (key, label, description, isActive, sortOrder, matchWeight). `category_matching_rules` table defines keyword matching rules (categoryKey, sourceField, matchType=contains/equals/starts_with/ends_with, searchTerm, priority, isActive). Rule engine (`server/services/categoryRuleEngine.ts`) evaluates rules by priority (lower=higher) against attendee source fields. Admin UI at `/admin/category-rules` with CategoryRulesPage: category CRUD, rule CRUD, rule tester tool, seed defaults button, backfill button. Backfill endpoint uses DB rules with fallback to legacy `normalizeAttendeeCategory()`. Matchmaking weights loaded dynamically from DB on startup via `setCategoryWeights()`/`setCategoryLabels()`. API: `GET/POST /api/admin/attendee-categories`, `PATCH/DELETE /api/admin/attendee-categories/:id`, `POST /api/admin/attendee-categories/seed-defaults`, `GET/POST /api/admin/category-rules`, `PATCH/DELETE /api/admin/category-rules/:id`, `POST /api/admin/category-rules/test`.
...
- **Access Control**: Granular, per-user permission system with 100+ boolean permission flags across 19 categories: Module Access, Events, Sponsors, Attendees, Meetings, Info Requests, Deliverables, Sponsorship Templates, Sponsor Dashboards, Reports, Email Center, Data Exchange, Data Backup, Branding & Settings, Users & Access Control, Matchmaking & Invitations, Demo Environment, Sensitive Data, and Account Controls. Includes role presets (Event Producer, Sales, Operations, Read Only) that can be applied from a dropdown. Access Control UI features collapsible permission groups with enable/disable all per group, and a permission audit log.
...
- **Sponsor/Attendee Matchmaking & Meeting Invitations**: Sponsors can discover and invite attendees to meetings. Controlled per-event via `matchmakingEnabled` flag. Schema: `meeting_invitations` table, `attendeeCategory` + `ticketType` on attendees, `matchmakingEnabled` + `maxInvitationsPerAttendee` + `invitationQuotas` on events. Matchmaking service (`server/services/matchmakingService.ts`) scores attendees by category weight (Practitioner=100, Gov/Non-Profit=70, Solution Provider=20), interest overlap with sponsor attributes, and role seniority. Sponsor Dashboard "Discovery" tab shows ranked attendees with match reasons, category counts, invitation quotas by level (Platinum=25, Gold=15, Silver=10, Bronze=5), and invite modal. Per-attendee invitation cap defaults to 6. Meeting invitation email contains secure token link to `/meeting-invitation/:token` where attendees can accept (pick time slot + location → creates meeting) or decline without needing an account. Eventzilla webhook normalizes `ticketType` to `attendeeCategory`. Admin routes: `GET /api/admin/meeting-invitations`, `PATCH /api/admin/meeting-invitations/:id`, `POST /api/admin/meeting-invitations/:id/resend`.
