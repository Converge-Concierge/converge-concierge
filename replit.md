...
- **Brevo Email Integration**: Transactional emails sent via Brevo (sib-api-v3-sdk) for 4 event types: `meeting_confirmation_attendee`, `meeting_notification_sponsor`, `info_request_notification_sponsor`, `info_request_confirmation_attendee`. Emails fire-and-forget on meeting creation and info request submission. HTML templates in `services/emailTemplates.js`. Sending logic in `services/emailService.js` (ESM, `"type": "module"`). Every send attempt logged to `email_logs` table including full `htmlContent` for preview/resend.
- **Admin Email Center** (`/admin/email-center`): Full email audit log browser. Summary cards (Total/Sent/Failed), filter bar (type, status, event, date range, keyword search), paginated table with View/Resend actions. Detail drawer shows all metadata + HTML preview (preview vs. source toggle). Resend action creates a new log entry with `resendOfId` linking back to original. "Send Test Email" modal lets admins dispatch any of the 4 template types to an arbitrary address. Backend routes: `GET /api/admin/email-logs`, `GET /api/admin/email-logs/:id`, `POST /api/admin/email-logs/:id/resend`, `POST /api/admin/email-logs/send-test`.
