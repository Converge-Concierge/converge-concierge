...
import { User, Calendar, Tag, FileText, Info } from "lucide-react";
          ...
          {isLoading ? (
            ...
            <div className="p-6 space-y-8 pb-12">
                ...
                <div className="flex flex-wrap gap-2">
                  {attendee.interests && attendee.interests.length > 0 ? (
                    attendee.interests.map((interest, i) => (
                      <Badge key={i} variant="secondary" className="px-2 py-0.5 rounded-full text-xs font-medium">
                        {interest}
                      </Badge>
                    ))
                  ) : (
                    <p className="text-sm text-muted-foreground italic">No interests recorded yet.</p>
                  )}
                </div>
              </section>
              ...
              <section className="space-y-4">
                ...
                <div className="text-sm bg-muted/30 p-4 rounded-lg border border-border/50 min-h-[60px] whitespace-pre-wrap">
                  {attendee.notes || <span className="text-muted-foreground italic">No internal notes for this attendee.</span>}
                </div>
              </section>
